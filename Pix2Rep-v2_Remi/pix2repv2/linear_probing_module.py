...
class Pix2RepLinearProbingModule(L.LightningModule):
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            images (torch.Tensor): Input batched image tensors of shape (B, C, H, W)

        Returns:
            torch.Tensor: Predicted logits of shape (B, num_classes, H, W)
        """
    ...
    def configure_optimizers(self):
        linear_probing_optim = torch.optim.AdamW(
            self.finetuning_layer.parameters(),
            lr=self.cfg.finetuning.linear_probing_lr_outconv,
        )
        return linear_probing_optim
